...
try {
  ...
  assert(anonymousSession.status === 200, "The Better Auth session endpoint must return 200.");
  assert((await anonymousSession.text()) === "null", "The smoke client must remain anonymous.");

  const home = await fetchWhenReady("/", { headers: { accept: "text/html" } });
  const menu = await fetchWhenReady("/menu", { headers: { accept: "text/html" } });
  const gallery = await fetchWhenReady("/gallery", { headers: { accept: "text/html" } });
  const privacy = await fetchWhenReady("/privacy", { headers: { accept: "text/html" } });
  const login = await fetchWhenReady("/admin/login", { headers: { accept: "text/html" } });
  const admin = await fetchWhenReady("/admin", {
    headers: { accept: "text/html" },
    redirect: "manual",
  });
  const missing = await fetchWhenReady("/missing-page", { headers: { accept: "text/html" } });
  ...
  assert(admin.status === 302, "The admin root must redirect without a session.");
  assert(admin.headers.get("location")?.endsWith("/admin/login"), "The admin redirect is invalid.");
  ...
  assert(publicScript.startsWith("/_client/"), "The public bundle uses an invalid prefix.");
  assert(adminScript.startsWith("/admin/_client/"), "The admin bundle uses an invalid prefix.");
  assert(publicScript !== adminScript, "The two applications must not share a client entry point.");
  assert(
    (await fetchWhenReady(publicScript)).status === 200,
    "The public client asset is missing.",
  );
  assert((await fetchWhenReady(adminScript)).status === 200, "The admin client asset is missing.");
