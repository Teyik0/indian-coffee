...
export const ReservationCreateSchema = v.object({
  fullName: v.pipe(v.string(), v.trim(), v.minLength(2, "Indiquez votre nom."), v.maxLength(100)),
  email: v.pipe(v.string(), v.trim(), v.email("Indiquez une adresse email valide.")),
  phone: v.pipe(
    v.string(),
    v.trim(),
  ...
  requestedTime: v.pipe(v.string(), v.regex(/^\d{2}:\d{2}$/, "Choisissez un horaire valide.")),
  occasion: v.optional(v.pipe(v.string(), v.maxLength(80)), ""),
  message: optionalMessage,
  consent: v.pipe(
    v.boolean(),
    v.check((value) => value, "Votre accord est nécessaire pour traiter la demande."),
  ),
  website: v.optional(v.string(), ""),
});

export const ReservationStatusSchema = v.picklist(["CONFIRMED", "DECLINED", "CANCELLED"]);
...
export type ReservationCreateInput = v.InferOutput<typeof ReservationCreateSchema>;
export type ReservationStatusUpdate = v.InferOutput<typeof ReservationStatusUpdateSchema>;
