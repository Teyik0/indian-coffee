...
export function MediaUploadForm() {
  ...
  const form = useForm({ schema: MediaUploadSchema, initialInput: { alt: "" } });
  ...
  return (
    <Form className="flex flex-col gap-6" of={form} onSubmit={submit}>
        ...
        <FormischField of={form} path={["alt"]}>
          {(field) => (
            <Field data-invalid={field.errors !== null}>
              <FieldLabel htmlFor="media-alt">Description accessible</FieldLabel>
              <Input
                {...field.props}
                ...
                value={field.input ?? ""}
              />
              <FieldDescription>Décrivez ce que montre la photo.</FieldDescription>
              {field.errors ? (
      ...
      <Button disabled={pending} type="submit">
        {pending ? <Spinner data-icon="inline-start" /> : <ImageUpIcon data-icon="inline-start" />}
